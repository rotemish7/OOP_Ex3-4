...
//					allUsers();	
	...
	public static void printLog(int id) {
		// how many plays, the max level.
		// the result in the levels
		try {
			...
			{
				if(resultSet.getInt("score") <= sc && resultSet.getInt("moves") <= mov)
				{
					System.out.println("level: "+resultSet.getInt("levelID")+", score: "+resultSet.getInt("score"));
//					System.out.println(ind+") Id: " + resultSet.getInt("UserID")+", level: "+resultSet.getInt("levelID")+", score: "+resultSet.getInt("score")+", moves: "+resultSet.getInt("moves")+", time: "+resultSet.getDate("time"));	
	...
	/**
	 * simply prints all the games as played by all the users (in the database).
	 * @param
	 * @param
	 * @return int
	 */
